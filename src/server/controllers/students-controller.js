...
exports.updateStudent = async (req, res) => {
  try {
    ...
    res.status(200).send({ message: "Entidade actualizado com sucesso!", data }); //sucesso
  } catch (error) {
    res.status(400).send({ message: "ERRO: Falha ao atualizar Entidade", error });
  }
};
...
exports.deleteStudent = async (req, res) => {
  try {
    ...
    res.status(400).send({ message: "ERRO: Falha ao eliminar Entidade", error });
  }
};
